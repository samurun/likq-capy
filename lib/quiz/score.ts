...
export function applyChoice(
  ...
  const next = { ...totals };
  for (const [tag, delta] of Object.entries(choice.tags) as [Tag, number][]) {
...
function profileMagnitude(profile: Record<Tag, number>): number {
  let sum = 0;
  for (const tag of TAGS) sum += profile[tag] * profile[tag];
...
export function pickArchetype(totals: Record<Tag, number>): ArchetypeId {
  ...
  for (const archetype of ARCHETYPE_LIST) {
    ...
    for (const tag of TAGS) {
      dot += (totals[tag] ?? 0) * archetype.profile[tag];
    }
...
export function archetypeBreakdown(
  ...
  return ARCHETYPE_LIST.map((a) => {
    let dot = 0;
    for (const tag of TAGS) dot += (totals[tag] ?? 0) * a.profile[tag];
